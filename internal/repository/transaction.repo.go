...
func (tr *TransactionRepository) GetAllByUserId(ctx context.Context, dbtx DBTX, id int) ([]model.TransactionResponse, error) {

	// jelaskan sql ini
	sql := `WITH MyWallet AS (
    ...
    FROM topups tp
    LEFT JOIN payment_methods pm ON tp.method_id = pm.id
    ...
    SELECT 
        tf.transaction_id, 
        tf.amount,
        CASE WHEN tf.receiver_wallet_id = (SELECT id FROM MyWallet) THEN 'Transfer Masuk' ELSE 'Transfer Keluar' END AS label,
        CASE WHEN tf.receiver_wallet_id = (SELECT id FROM MyWallet) THEN 'in' ELSE 'out' END AS flow,
        p.full_name AS counterparty_name,
        p.phone AS counterparty_phone
    FROM transfers tf
    JOIN wallets w_lawan ON w_lawan.id = CASE 
        WHEN tf.receiver_wallet_id = (SELECT id FROM MyWallet) THEN tf.sender_wallet_id 
        ELSE tf.receiver_wallet_id 
    END
    JOIN profiles p ON p.user_id = w_lawan.user_id
    WHERE tf.sender_wallet_id = (SELECT id FROM MyWallet) OR tf.receiver_wallet_id = (SELECT id FROM MyWallet)
	)
    ...
    COALESCE(tp.counterparty_name, tf.counterparty_name) AS counterparty_name,
    COALESCE(tp.counterparty_phone, tf.counterparty_phone) AS counterparty_phone,
    trx.status, 
    trx.created_at
	FROM transactions trx
	LEFT JOIN MyTopups tp ON trx.id = tp.transaction_id
	LEFT JOIN MyTransfers tf ON trx.id = tf.transaction_id
	WHERE tp.transaction_id IS NOT NULL OR tf.transaction_id IS NOT NULL
	...
	for rows.Next() {
		...
		err := rows.Scan(&item.TransactionID, &item.ReferenceCode, &item.TransactionType, &item.TransactionLabel, &item.FlowType, &item.Amount, &item.CounterpartyName, &item.CounterpartyPhone, &item.Status, &item.CreatedAt)
		if err != nil {
			return nil, err
		}
